# make_benchmark.py
# Usage:
#   python make_benchmark.py --input LO_All.tsv --out benchmark.csv --n_cases 20 --seed 42
...
class ExtraConstraint:
    ...
    op: str            # ">=" or "<="
    threshold: float   # rounded value (2 decimals)
...
def quantile_threshold(values: List[float], q: float) -> float:
    # stable quantile; round to 2 decimals to reduce floating disputes
...
def build_prompt(smiles10: List[str], constraints: List[ExtraConstraint]) -> str:
    ...
    for c in constraints:
        # keep property names simple/standard
        # (MolLogP, TPSA, RotB, MolWt)
...
def oracle_select(smiles10: List[str], props_map: Dict[str, Dict[str, float]], constraints: List[ExtraConstraint]) -> List[str]:
    selected = []
    for s in smiles10:
        p = props_map[s]
        ...
        for c in constraints:
            if not apply_extra(p, c):
                ...
                break
        if ok:
            selected.append(s)  # original string
    return selected


# ---------- Case generation with k in [1,5] ----------
EXTRA_POOL = [
    ("MolLogP", ">=", [0.4, 0.5, 0.6, 0.7]),
    ("MolLogP", "<=", [0.3, 0.4, 0.5, 0.6]),
    ("TPSA", "<=",   [0.3, 0.4, 0.5, 0.6]),
    ("TPSA", ">=",   [0.4, 0.5, 0.6, 0.7]),
    ("RotB", "<=",   [0.3, 0.4, 0.5, 0.6]),
    ("RotB", ">=",   [0.4, 0.5, 0.6, 0.7]),
    ("MolWt", "<=",  [0.3, 0.4, 0.5, 0.6]),
    ("MolWt", ">=",  [0.4, 0.5, 0.6, 0.7]),
]
...
def try_make_case(
    ...
    Build one case:
      - sample 10 SMILES
      - compute props
      - sample 2-3 extra constraints (we use 2 by default; can become 3 if needed)
      - adjust constraints/thresholds until selected_count in [1,5]
    """
    for attempt in range(1, max_attempts + 1):
        smiles10 = rng.sample(smiles_pool, 10)

        # compute props; if any invalid SMILES, resample (simple + strict)
        props_map = {}
        valid = True
        for s in smiles10:
            p = calc_props(s)
        ...
        # decide number of extra constraints: mostly 2, occasionally 3
        ...
        for _ in range(n_extra):
            ...
            for _inner in range(20):
                ...
                constraints.append(ExtraConstraint(name=name, op=op, threshold=thr))
                break
        ...
        if k_min <= k <= k_max:
            ...
            answer = "\n".join(selected)  # empty string allowed if none, but we filtered to [1,5]
            meta = CaseMeta(
                assay_id=assay_id,
        ...
        # if out-of-range, loop and resample constraints/smiles
...
def main():
    ...
    ap.add_argument("--input", required=True, help="Input TSV path, e.g. CARA/Task/LO_All.tsv")
    ...
    # group unique SMILES per assay (preserve original strings)
    g = df.groupby(args.assay_col)[args.smiles_col].apply(lambda s: list(dict.fromkeys(s.tolist())))
    eligible = [(assay, smi_list) for assay, smi_list in g.items() if len(smi_list) >= 10]

    if len(eligible) == 0:
        raise RuntimeError("No eligible assays with >=10 unique SMILES found.")

    # shuffle eligible assays deterministically
    ...
    while len(rows) < args.n_cases and assay_try < args.max_assay_tries:
        ...
        # allow reuse if dataset small, but prefer unique assays
    ...
    if len(rows) < args.n_cases:
        raise RuntimeError(f"Only generated {len(rows)} cases; try increasing --max_assay_tries or relaxing constraints.")
