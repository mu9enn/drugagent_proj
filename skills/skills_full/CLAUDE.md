...
auto-generated-skills/  ← Crystallized skills from prior runs; check skill-index.md
                         Contains L1_tools/ and L2_workflows/ subdirs (currently empty)
```

All skill files live under `.claude/skills/`.
...
### L2 — Workflows
Step-by-step protocols in `.claude/skills/L2_workflows/`:
...
### LR — Research
Research workflows in `.claude/skills/LR_research/workflows/`:
- `deep-research.md`

### L1 — Tools
Individual tool specs in `.claude/skills/L1_tools/` (60 directories, each containing a `SKILL.md`). Some directories include supplementary files — see notes below. Full list:
...
- `molclaw-diffdock-auto` — DiffDock blind docking
...
- **`system_prompt_FULL.md`** — full agent system prompt (18 KB); defines the 5-phase execution framework, data integrity rules, file naming conventions, and experiential learning configuration ("Full-EL" mode)
- **`batch_test.py`** — Python harness for batch-testing drug discovery questions; reads questions from markdown files, creates isolated working directories per question, and invokes the `claude` CLI with the system prompt either concurrently or sequentially
...
- **Docking constraints:** Vina/QuickVina scores MUST be negative. Minimum box size is 25 Å per dimension. Progressive enlargement on failure: 25 → 30 → 40 → 50 Å before switching to DiffDock/KarmaDock.
